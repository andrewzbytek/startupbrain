...
def render_dashboard():
    ...
    if sections:
        # Iterate in rows of 3
        for row_start in range(0, len(sections), 3):
            row_sections = sections[row_start : row_start + 3]
            cols = st.columns(3)
            for col, section in zip(cols, row_sections):
                with col:
                    pos = section["current_position"]
                    has_content = pos and pos != "[Not yet defined]"
                    dot_class = "section-dot-green" if has_content else "section-dot-gray"
                    display_name = html.escape(section["name"])
                    if has_content and len(pos) > 150:
                        preview = html.escape(_escape_latex(pos[:150])) + "..."
                    elif has_content:
                        preview = html.escape(_escape_latex(pos))
                    else:
                        preview = "<em>Not yet defined</em>"
                    # Card wraps both header and preview content
                    st.markdown(
                        f'<div class="dashboard-card">'
                        f'<span class="{dot_class}"></span>'
                        f' <strong>{display_name}</strong>'
                        f'<div style="margin-top:0.5rem;font-size:0.85rem;color:var(--text-secondary);">'
                        f'{preview}</div>'
                        f'</div>',
    ...
    # =====================================================================
    # Secondary panels: 2-column layout
    # =====================================================================
    ...
    with left_col:
        ...
        if hypotheses:
            ...
            for i, h in enumerate(active):
                ...
                with st.expander(f'{html.escape(h["text"][:60])}', expanded=False):
                    st.markdown(badge_html, unsafe_allow_html=True)
                    st.markdown(f"**Tracked:** {h['date']}")
                    st.markdown(f"**Test:** {_escape_latex(h['test'])}")
                    st.markdown(f"**Evidence:** {_escape_latex(h['evidence'])}")
    ...
    with right_col:
        ...
        if has_source_feedback:
            ...
            # Fall back to recurring themes from MongoDB, then doc, then empty
    ...
    with act_col2:
        if doc:
            if st.button("Download Full Context", use_container_width=True, key="dash_export_btn"):
    ...
    # Display evolution result below action row if present
