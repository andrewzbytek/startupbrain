...
- [ ] If ADD_CONTACT or UPDATE_CONTACT actions are present, does each include required fields: Name (bold), Organization, Type, Status, Context, Next step? (BLOCKING)
- [ ] If ADD_CONTACT is present, is the contact name genuinely new (not already in the contacts section)? Should be UPDATE_CONTACT instead. (BLOCKING)
